...
### [2024-05-06] 重大升级：智能化与多接口扩展
🚀 智能拆分器 2.0：彻底重制了 Trucy 文本拆分器 (5路 & 10路)，使其成为逻辑处理核心。
三路同步输出：每个拆分片段现在同步提供 字符串 (String)、整数 (Int)、浮点数 (Float) 三种接口。
智能数字提取：内置正则表达式引擎，自动识别片段中的数字（如“04.主角”自动提取 Int: 4），逻辑适配同 Excel 节点。
分隔符全覆盖：分隔符列表大幅扩充，涵盖了几乎所有特殊标点（~!@#$%^&*()-+_{}[]|<> 等）。
🎯 ID 提取器升级：现已完美支持 2位字符 ID（如 X1），第 3 位字符变为可选 Ignore。
📁 架构终极整合：完成 trucy_toolkit.py 统合，将所有工具节点归于一处，显著提升加载速度。
🛡️ 稳定性修复：修正了 TrucyAssetGrid5 的映射问题，确保 5 图预览节点在所有工作流中稳定运行。
