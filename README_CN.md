...
### [2024-05-06] 生产级重构：UI一致性与报错处理机制
🚀 UI/UX 逻辑统一：深度对齐了“音频加载器”与“文本加载器”的菜单顺序及参数命名，确保操作体感高度一致。
🔢 智能索引切换：为文本和音频加载器同步增加了 index_mode (0-based / 1-based) 切换功能，完美适配 Excel 序号或循环节点。
🛡️ 严格越界保护：
文本节点：当索引超出文件夹范围时，输出 "N/A"，不再自动循环回到起点，方便排查脚本进度。
音频节点：当索引超出范围时，自动生成 1秒 1000Hz 标准提示音 (Beep)。通过听觉反馈直接提醒素材缺失。
🔄 Excel 实时同步：优化了 Excel 读取节点的缓存机制。现在节点会监测文件修改时间，只要在外部保存 Excel，ComfyUI 运行即会自动读取最新内容。
⚡ 工具箱性能整合：完成 trucy_toolkit.py 的最终封装，显著提升插件加载速度并净化了文件目录。
