...
# 核心算法基类 (封装所有核心逻辑)
# ======================================================================
class BaseTrucyKleinEncode:
    def encode_klein_logic(self, clip, prompt, width, height, non_base_alignment, rsa_value, base_target, count, vae=None, base_mask=None, **kwargs):
        if vae is None:
    ...
    def INPUT_TYPES(cls):
        inputs = {
            "required": {
                ...
                "base_target": (["None"] + [f"img{i}" for i in range(1, 11)], {"default": "img1"}),
            },
            "optional": {
                "vae": ("VAE",),
                "base_mask": ("MASK",),  
